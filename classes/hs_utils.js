...
function getContactProperties(accessToken, id) {
  options = {
    method: 'GET',
    uri:
      'https://api.hubapi.com/contacts/v2/properties?access_token=' +
      accessToken
  };
...
function formatContactProperties(d, id) {
  ...
  upsertObj.property_group = property_group.filter(function(
    item,
    index,
    inputArray
  ) {
...
function createWorkflow(token, id, data) {
  ...
  options.body = JSON.stringify({
    name: 'HubSlacker',
    type: 'DRIP_DELAY',
    enabled: 'truhttps://hs-slack-leadnotify.herokuapp.com/api/hubspot/leade',
    actions: [
      {
        type: 'WEBHOOK',
        url: '',
